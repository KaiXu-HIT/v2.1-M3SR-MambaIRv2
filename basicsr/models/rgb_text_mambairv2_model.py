...
class RGBTextMambaIRv2Model(MambaIRv2Model):
    """Stage-1 B2 training and tiled-evaluation wrapper."""

    def setup_optimizers(self):
        # B2 change: summarize frozen CLIP parameters once instead of emitting
        # one warning per tensor through the generic SRModel implementation.
        ...
        get_root_logger().info(
            f'B2 optimizer parameters: {trainable_count:,d} trainable; '
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(self.lq, text=self.text)
    ...
    def test(self):
        ...
        test_net.eval()
        with torch.no_grad():
            # B2 change: encode the global caption once, then reuse its FiLM
            # condition for every image tile.
            text_condition = test_net.encode_text(
                self.text, rgb.shape[0], rgb.device, rgb.dtype)
            outputs = [
                test_net(rgb[..., top, left], text_condition=text_condition)
